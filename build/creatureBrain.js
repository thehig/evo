...
import { Creature, DietType } from "./grid.js";
export class CreatureBrain {
    constructor(creature, neuralNetwork) {
        ...
        if (neuralNetwork) {
            ...
            this.neuralNetwork = NeuralNetworkGenerator.generateRandom(20, // Input size: 8 food + 8 threat + 2 internal + 2 environmental = 20
    ...
    gatherSensoryInput(grid) {
        ...
        for (const dir of directions) {
            ...
            input[`${dir.name}Food`] = foodStrength;
            input[`${dir.name}Threat`] = threatStrength;
        }
        return input;
    }
    isFood(entity) {
        if (entity.type === "Plant") {
            return (this.creature.dietType === DietType.HERBIVORE ||
                this.creature.dietType === DietType.OMNIVORE);
        }
        if (entity instanceof Creature && entity !== this.creature) {
            return (this.creature.dietType === DietType.CARNIVORE ||
                this.creature.dietType === DietType.OMNIVORE);
        }
        return false;
    }
    isThreat(entity) {
        if (entity instanceof Creature && entity !== this.creature) {
            ...
            return (entity.dietType === DietType.CARNIVORE ||
                entity.dietType === DietType.OMNIVORE);
        }
        return false;
    }
    makeBehaviorDecision(sensoryInput) {
    ...
    shouldAttemptReproduction(grid) {
    ...
    static createOffspringBrain(parent1, parent2) {
        let offspringNetwork;
        // Try to crossover if networks are compatible
        try {
            offspringNetwork = NeuralNetwork.crossover(parent1.neuralNetwork, parent2.neuralNetwork);
            // Apply mutation
            if (Math.random() < 0.8) {
            ...
            offspringNetwork = NeuralNetworkGenerator.generateRandom(20, 11, Math.random());
        }
    ...
    static deserialize(creature, data) {
