...
export function parseSeed(seed) {
    if (typeof seed !== "string" || seed.length < 7) {
        console.error("Invalid seed: Seed must be a string of at least 7 characters.");
        return null;
    }
    let dietType;
    ...
    if (!isNaN(perceptionArcNum) &&
        perceptionArcNum >= 0 &&
        perceptionArcNum <= 8) {
        if (perceptionArcNum === 0)
            perceptionArc = 0; // Line
        else if (perceptionArcNum === 8)
            perceptionArc = 360; // Omni
        else
            perceptionArc = perceptionArcNum * ARC_STEP;
    }
    else {
    ...
    if (perceptionType === PerceptionType.NONE &&
        !["V", "S", "E"].includes(perceptionTypeChar))
        console.error(`Invalid perception type '${perceptionTypeChar}'. Defaulting.`);
    if (activityCycle === ActivityCycle.NONE &&
