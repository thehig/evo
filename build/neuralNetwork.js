...
export class NeuralNetwork {
    ...
    static fromWeightsAndBiases(weights, biases, config) {
    ...
    getActivationFunction() {
        switch (this.config.activationFunction) {
            ...
            default:
                return (x) => 1 / (1 + Math.exp(-x)); // Default to sigmoid
        }
    }
    mutate() {
        ...
        return NeuralNetwork.fromWeightsAndBiases(newWeights, newBiases, this.config);
    }
    static crossover(parent1, parent2) {
        if (parent1.weights.length !== parent2.weights.length) {
            throw new Error("Neural networks must have the same architecture for crossover");
        }
        const newWeights = [];
        const newBiases = [];
        // Crossover weights
        ...
        return NeuralNetwork.fromWeightsAndBiases(newWeights, newBiases, parent1.config);
    }
    serialize() {
    ...
    static deserialize(data) {
        ...
        const nn = new NeuralNetwork(config);
        nn.weights = parsed.weights.map((w) => {
            const matrix = new Matrix(w.length, w[0].length);
            matrix.data = w;
            return matrix;
        });
        nn.biases = parsed.biases.map((b) => {
...
export class NeuralNetworkGenerator {
    ...
    static evolveArchitecture(parent) {
        ...
        if (Math.random() < 0.3) {
            ...
            if (Math.random() < 0.5 && newConfig.hiddenLayers.length < 5) {
                // Add layer
                const insertIndex = Math.floor(Math.random() * (newConfig.hiddenLayers.length + 1));
                const newSize = Math.floor((newConfig.inputSize + newConfig.outputSize) / 2 +
                    (Math.random() - 0.5) * 10);
                newConfig.hiddenLayers.splice(insertIndex, 0, Math.max(1, newSize));
            }
            else if (newConfig.hiddenLayers.length > 1) {
                // Remove layer
                const removeIndex = Math.floor(Math.random() * newConfig.hiddenLayers.length);
        ...
        if (Math.random() < 0.4) {
            for (let i = 0; i < newConfig.hiddenLayers.length; i++) {
                if (Math.random() < 0.3) {
                    ...
                    newConfig.hiddenLayers[i] = Math.max(1, newConfig.hiddenLayers[i] + change);
                }
            }
        }
