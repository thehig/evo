...
export class CreatureFactory {
    static createRandomCreature(x = -1, y = -1) {
        ...
        const activityCycle = Math.random() < 0.5 ? ActivityCycle.DIURNAL : ActivityCycle.NOCTURNAL;
        ...
        const seed = this.generateRandomSeed(dietType, movementSpeed, perceptionType, activityCycle, perceptionRange);
        ...
        const creature = new Creature(symbol, color, "Creature", x, y, dietType, movementSpeed, activityCycle, perceptionType, perceptionRange, perceptionAngle, perceptionArc, 100, // Initial energy
        seed, brain);
        // Update brain's creature reference
        brain.setCreature(creature);
        return creature;
    }
    static createCreatureWithComplexity(x = -1, y = -1, complexity = 0.5, dietType) {
        // Use provided diet type or random
        const finalDietType = dietType ||
            [DietType.HERBIVORE, DietType.CARNIVORE, DietType.OMNIVORE][Math.floor(Math.random() * 3)];
        // Determine symbol and color based on diet
        let symbol;
        let color;
        ...
        const seed = this.generateRandomSeed(finalDietType, movementSpeed, perceptionType, activityCycle, perceptionRange);
        // Create neural network with specified complexity
        const neuralNetwork = NeuralNetworkGenerator.generateRandom(20, 11, complexity);
        const brain = new CreatureBrain(new Creature(), neuralNetwork);
        const creature = new Creature(symbol, color, "Creature", x, y, finalDietType, movementSpeed, activityCycle, perceptionType, perceptionRange, perceptionAngle, perceptionArc, 100, seed, brain);
        brain.setCreature(creature);
        return creature;
    }
    static generateRandomSeed(dietType, movementSpeed, perceptionType, activityCycle, perceptionRange) {
    ...
    static createSpeciesPopulation(count, dietType, complexity = 0.5) {
        const population = [];
        for (let i = 0; i < count; i++) {
            const creature = this.createCreatureWithComplexity(-1, -1, complexity, dietType);
