...
describe("Simulation - Reproduction Dynamics", () => {
    ...
    it("two creatures should reproduce when conditions are met", () => {
        const parentA = new Creature("A", "red", "Creature", 0, 0, DietType.HERBIVORE, 1, ActivityCycle.DIURNAL, PerceptionType.VISION, 1, 0, 45, 200, "PA_SEED");
        const parentB = new Creature("B", "blue", "Creature", 1, 0, DietType.HERBIVORE, 1, ActivityCycle.DIURNAL, PerceptionType.VISION, 1, 0, 45, 200, "PB_SEED");
        ...
        expect(addEntitySpy.mock.calls.some((call) => call[0] instanceof Creature &&
            call[0] !== parentA &&
            call[0] !== parentB)).toBe(true);
